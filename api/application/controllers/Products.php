...
class Products extends CI_Controller{
    ...
    public $_table_column_array = ['storeId','cid','cover','descriptions_en','descriptions_ar','notes_en','notes_ar','price','price_type','rating','veg','name_en','name_ar','variations','status','size','internal_tax'];
    public $_table_column_edit = ['id','storeId','cid','cover','descriptions_en','descriptions_ar','notes_en','notes_ar','price','price_type','rating','veg','name_en','name_ar','variations','status','size','internal_tax'];
    public $required = ['id'];
    ...
    public function getAllProductsbyAgent(){
        ...
        $auth  = $this->input->get_request_header('Basic');
        if($auth && $auth == $this->config->item('encryption_key')){
            $data = $this->Product_model->getAllProductsbyAgent();
            if($data != null){
                echo $this->json->response($data,$this->_OKmessage,$this->_statusOK);
            }else{
                echo $this->json->response($this->db->error(),$this->_Errmessage,$this->_statusErr);
    ...
    public function getAllByStoreId(){
        $agent = $this->input->request_headers();
        // $saveLogInfo = array(
        //     'url' => $this->uri->uri_string(),
        //     'agent' => json_encode($agent),
        ...
        if($auth && $auth == $this->config->item('encryption_key')){
            ...
            if(isset($data) && !empty($data)){
                ...
                $result = $this->Product_model->getAllByStoreId($_POST['id']);
                if($result != null){
                    echo $this->json->response($result,$this->_OKmessage,$this->_statusOK);
                }else{
                    echo $this->json->response($this->db->error(),$this->_Errmessage,$this->_statusErr);
