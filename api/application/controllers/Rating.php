...
class Rating extends CI_Controller{
    ...
    public $_table_column_array = ['uid','oid','did','sid','drate','srate','msg','way','status','timestamp'];
    public $_table_column_edit = ['id','uid','oid','did','sid','drate','srate','msg','way','status','timestamp'];
    public $required = ['id'];
    ...
    //   $this->Rating_model->saveUserLogs($saveLogInfo);
        ...
        if($auth && $auth == $this->config->item('encryption_key')){
            ...
            if(isset($data) && !empty($data)){
                ...
                $result = $this->Rating_model->getByStoreId($_POST['sid']);
                if($result != null){
                    echo $this->json->response($result,$this->_OKmessage,$this->_statusOK);
                }else{
                    echo $this->json->response($this->db->error(),$this->_Errmessage,$this->_statusErr);
    ...
    public function getFromStoreCount(){
        $agent = $this->input->request_headers();
    //     $saveLogInfo = array(
    //         'url' => $this->uri->uri_string(),
    //         'agent' => json_encode($agent),
    ...
    //   $this->Rating_model->saveUserLogs($saveLogInfo);
        ...
        if($auth && $auth == $this->config->item('encryption_key')){
            ...
            if(isset($data) && !empty($data)){
                ...
                $result = $this->Rating_model->getFromStoreCount($_POST['where']);
                $rates = $this->Rating_model->getWhereStoreIds($_POST['where']);
                if($result != null){
                     $returnValue = array(
                        'total' => $result,
                        'rating' => $rates,
    ...
    public function getFromDriverCount(){
        $agent = $this->input->request_headers();
    //     $saveLogInfo = array(
    //         'url' => $this->uri->uri_string(),
    //         'agent' => json_encode($agent),
    ...
    //   $this->Rating_model->saveUserLogs($saveLogInfo);
        ...
        if($auth && $auth == $this->config->item('encryption_key')){
            ...
            if(isset($data) && !empty($data)){
                ...
                $result = $this->Rating_model->getFromDriverCount($_POST['where']);
                $rates = $this->Rating_model->getWhereDriverIds($_POST['where']);
                if($result != null){
                     $returnValue = array(
                        'total' => $result,
                        'rating' => $rates,
