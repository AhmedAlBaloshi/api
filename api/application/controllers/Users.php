...
class Users extends CI_Controller
{
    ...
    public function adminHomeByAgent()
    {
        $agent = $this->input->request_headers();
        //     $saveLogInfo = array(
        //         'url' => $this->uri->uri_string(),
        ...
        if ($auth && $auth == $this->config->item('encryption_key')) {
            $allUsers = $this->Users_model->countAllUsers();
            $orders = $this->Order_model->getAdminTopByAgent();
            $stores = $this->Stores_model->getAllStoresbyAgent();
            //$allorders = $this->Order_model->adminAllOrders();
            $allorders = $this->Order_model->countAllOrders();
            foreach ($allUsers as $keys) {
                $keys->password = null;
    ...
    public function getFcmById()
    {
        ...
        if ($auth && $auth == $this->config->item('encryption_key')) {
            ...
            if (isset($data) && !empty($data)) {
                ...
                $result = get_column('users', 'id', $_POST['id'], 'fcm_token');
                if ($result != null) {
                    echo $this->json->response($result, $this->_OKmessage, $this->_statusOK);
                } else {
                    echo $this->json->response($this->db->error(), $this->_Errmessage, $this->_statusErr);
    ...
    public function registerUser()
    {
        ...
        if ($auth && $auth == $this->config->item('encryption_key')) {
            ...
            //echo $this->json->response('No Token Found',$this->_Errmessage,$this->_statusErr);
            echo $this->json->response($this->input->get_request_header('Basic'));
        }
    }
    ...
    public function deleteUser()
    {
        ...
        $auth  = $this->input->get_request_header('Basic');
        if ($auth && $auth == $this->config->item('encryption_key')) {
            $saveResult = $this->Users_model->save_deleted_users($_POST);
            if ($saveResult != null) {
                $deleteResult = $this->Users_model->delete_users($_POST['original_id'], $_POST['email']);
                if ($deleteResult != null) {
                    echo $this->json->response($deleteResult, $this->_OKmessage, $this->_statusOK);
                } else {
                    echo $this->json->response(['error' => 'Could not delete user.'], $this->_Errmessage, $this->_statusErr);
                }
            } else {
                echo $this->json->response(['error' => 'Something Went Wrong.'], $this->_Errmessage, $this->_statusErr);
            }
    ...
    public function validatePhone()
    {
        ...
        if ($auth && $auth == $this->config->item('encryption_key')) {
            ...
            if (isset($data) && !empty($data)) {
                ...
                $result = $this->Users_model->phoneExist($_POST['phone']);
                if ($result != null) {
                    echo $this->json->response(['message' => 'Mobile Phone Already Register in Database'], $this->_Errmessage, $this->_statusErr);
                } else {
                    echo $this->json->response(true, $this->_OKmessage, $this->_statusOK);
    ...
    public function storeCapacitorFcm()
    {
        $agent = $this->input->request_headers();
        //     $saveLogInfo = array(
        //         'url' => $this->uri->uri_string(),
    ...
    public function storeOneSignalFcm()
    {
        $agent = $this->input->request_headers();
        //     $saveLogInfo = array(
        //         'url' => $this->uri->uri_string(),
    ...
    public function storeGeneralData()
    {
        $agent = $this->input->request_headers();
        //     $saveLogInfo = array(
        //         'url' => $this->uri->uri_string(),
    ...
    public  function sendNotification()
    {
        ...
        if ($auth && $auth == $this->config->item('encryption_key')) {
            ...
            foreach ($_POST['users'] as $user) {
                $response = $this->Users_model->sendNotification($user, $_POST);
            }
            // if(json_encode($response)!= 'false'){
            echo $this->json->response(json_encode($response), $this->_OKmessage, $this->_statusOK);
        // }
            // else{
            //     echo $this->json->response(['message' => 'Something went wrong1'], $this->_Errmessage, $this->_statusErr);
            // }
