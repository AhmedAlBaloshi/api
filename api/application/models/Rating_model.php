...
class Rating_model extends Main_model
{
    ...
    {
        $this->db->select('rat.id, rat.uid as uid,rat.oid as oid,rat.did as did,rat.msg as msg,rat.way as way,rat.status as status,rat.timestamp as timestamp, user.full_name as fname,user.last_name as lname,user.cover as cover');
        $this->db->from("rating as rat");
        $this->db->join('users as user', 'rat.uid = user.id');
        $this->db->where($que);
        $this->db->where('rat.deleted_at', null);
    ...
    public function deleteList($id)
    {
        ...
        $this->db->where('id', $id);
        return $this->db->update($this->table_name, $data); // return $this->delete($this->table_name,$where);
    }
